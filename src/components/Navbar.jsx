...
export default function Navbar() {
  ...
  const isAdmin = profile?.role === 'group_admin' || isMaster
    ...
    <>
      ...
      <nav className="bottom-nav" style={s.bottomNav}>
        <Link to="/" style={s.navItem}>
          <svg width="21" height="21" fill="none" stroke={path==='/' ? '#c8410a' : '#bbb'} strokeWidth="2" viewBox="0 0 24 24"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
        ...
        <Link to="/mesajlar" style={s.navItem}>
          ...
          <span style={{...s.navLabel, color:path==='/mesajlar' ? '#c8410a' : '#bbb'}}>Mesajlar</span>
        </Link>
        {isAdmin ? (
          <Link to="/admin" style={s.navItem}>
            <svg width="21" height="21" fill="none" stroke={path==='/admin' ? '#c8410a' : '#bbb'} strokeWidth="2" viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>
            <span style={{...s.navLabel, color:path==='/admin' ? '#c8410a' : '#bbb'}}>Yönetim</span>
          </Link>
        ) : (
          <button onClick={handleLogout} style={{...s.navItem, background:'none', border:'none', cursor:'pointer', padding:'6px 12px'}}>
            <svg width="21" height="21" fill="none" stroke="#bbb" strokeWidth="2" viewBox="0 0 24 24"><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>
...
const s = {
  ...
  brandIcon: { width:40, height:40, borderRadius:12, background:'#c8410a', color:'#fff', display:'flex', alignItems:'center', justifyContent:'center', fontSize:18, fontWeight:800 },
  ...
  link: { color:'#777', textDecoration:'none', fontSize:13, padding:'7px 14px', borderRadius:9, fontWeight:500 },
  linkActive: { color:'#c8410a', background:'#fef0ed', fontWeight:600 },
  addBtn: { background:'#c8410a', color:'#fff', textDecoration:'none', fontSize:13, padding:'9px 18px', borderRadius:9, fontWeight:600, marginLeft:8 },
  userArea: { display:'flex', alignItems:'center', gap:10, flexShrink:0, background:'#f5f4f0', border:'1px solid #ece9e4', borderRadius:24, padding:'5px 14px 5px 7px' },
  ...
  userName: { fontSize:13, color:'#555', fontWeight:500, maxWidth:120, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' },
  logoutBtn: { color:'#aaa', background:'none', border:'none', fontSize:12, cursor:'pointer', padding:'0 0 0 8px', borderLeft:'1px solid #ece9e4', marginLeft:4 },
