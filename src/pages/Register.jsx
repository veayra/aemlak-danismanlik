...
export default function Register() {
  const [form, setForm] = useState({ full_name:'', company:'', phone:'', email:'', password:'', group_code:'' })
  ...
  const handleRegister = async (e) => {
    ...
    const { data: group, error: groupErr } = await supabase
      .from('groups').select('id, name').eq('code', form.group_code.toUpperCase()).eq('status', 'active').single()

    if (groupErr || !group) {
      setError('Geçersiz grup kodu. Lütfen yöneticinizden alın.')
      setLoading(false)
      return
    }
  ...
  if (success) return (
    <div style={s.page}>
      <div style={s.card}>
        ...
        <p style={s.successText}>Grup yöneticiniz hesabınızı onayladıktan sonra giriş yapabilirsiniz.</p>
  ...
  return (
    <div style={s.page}>
      <div style={s.card}>
        ...
        <p style={s.sub}>Grup kodunuzla kayıt olun</p>
        <form onSubmit={handleRegister} style={{marginTop:20}}>
          <div style={s.field}>
            <label style={s.label}>Grup Kodu *</label>
            <input style={{...s.input, borderColor:'#c8410a', background:'#fef0ed'}}
              value={form.group_code} onChange={set('group_code')} required
              placeholder="Yöneticinizden alın (örn: ABC123)" />
            <p style={{fontSize:11,color:'#aaa',marginTop:4}}>Grup kodunuz yoksa yöneticinize ulaşın</p>
          </div>
          {[
        ...
        <p style={s.footer}>Hesabınız var mı? <Link to="/giris" style={s.link}>Giriş Yapın</Link></p>
      </div>
    </div>
  )
}
...
const s = {
  ...
  card: { width:'100%', maxWidth:420, background:'#fff', border:'1px solid #ece9e4', borderRadius:20, padding:'32px 28px', boxShadow:'0 4px 24px rgba(0,0,0,0.06)' },
  ...
  input: { width:'100%', padding:'12px 14px', background:'#f5f4f0', border:'1px solid #e0ddd8', borderRadius:10, fontSize:15, color:'#1a1a1a', outline:'none', fontFamily:'inherit' },
