  ...
  return (
    <>
      <style>{`
        ...
        .lw { position:fixed; inset:0; display:flex; flex-direction:column; overflow:hidden; }
        ...
        .lc { position:relative; z-index:2; flex:1; display:flex; flex-direction:column; justify-content:center; padding:0 22px; overflow:hidden; }
        ...
        .l-foot a, .l-foot button { color:#fff; text-decoration:none; font-weight:700; background:none; border:none; cursor:pointer; font-size:14px; padding:0; }
        .l-pc-text { display:none; position:absolute; left:0; bottom:0; width:58%; z-index:2; flex-direction:column; padding:44px; background:linear-gradient(to top, rgba(0,0,0,0.8) 0%, transparent 100%); }
        @media(min-width:768px) {
          ...
          .l-foot { color:#aaa; }
          .l-foot a, .l-foot button { color:#c8410a; }
      ...
      <div className="lw">
        ...
        <div className="l-pc-text">
          <div className="l-badge" style={{marginBottom:14}}><div className="l-bdot"/><span className="l-btxt">Yalnızca yetkili emlakçılara özel</span></div>
        ...
        <div className="lc">
          <div className="l-logo">
            ...
            <div><div className="l-logo-name">A Takımı</div><div className="l-logo-sub">Profesyonel emlak platformu</div></div>
          </div>

          {!resetMode ? (
            <>
              <div className="l-badge" style={{marginBottom:10}}><div className="l-bdot"/><span className="l-btxt">Yetkili Emlakçılara Özel</span></div>
              ...
              <div className="l-form">
                ...
                <button className="l-btn" onClick={handleLogin} disabled={loading}>{loading ? 'Giriş yapılıyor...' : 'Giriş Yap →'}</button>
              </div>
              <p className="l-foot">
                <button onClick={() => { setResetMode(true); setError('') }}>Şifremi Unuttum</button>
                {' · '}
            ...
            <>
              <h1 className="l-title" style={{fontSize:22}}>Şifre Sıfırla</h1>
              <p className="l-sub">E-posta adresinize sıfırlama linki göndereceğiz</p>
              {resetSent ? (
                <div style={{background:'rgba(34,197,94,0.15)',border:'1px solid rgba(34,197,94,0.3)',borderRadius:12,padding:16,marginTop:16}}>
                  <p style={{color:'#4ade80',fontSize:14,fontWeight:600,marginBottom:4}}>✓ E-posta gönderildi!</p>
                  <p style={{color:'rgba(255,255,255,0.6)',fontSize:13}}>Gelen kutunuzu kontrol edin ve linke tıklayın.</p>
                </div>
              ) : (
                <div className="l-form" style={{marginTop:16}}>
                  <label className="l-lbl">E-posta Adresiniz</label>
                  <input className="l-inp" type="email" value={resetEmail} onChange={e=>setResetEmail(e.target.value)} placeholder="ornek@mail.com" />
                  {error && <div className="l-err">{error}</div>}
                  <button className="l-btn" onClick={handleReset} disabled={resetLoading}>{resetLoading ? 'Gönderiliyor...' : 'Sıfırlama Linki Gönder'}</button>
                </div>
              )}
              <p className="l-foot" style={{marginTop:14}}>
                <button onClick={() => { setResetMode(false); setResetSent(false); setError('') }}>← Giriş ekranına dön</button>
              </p>
            </>
          )}
        </div>
